    ...
    def action_download_attendance(self):
        ...
        for info in self:
            ...
            if conn:
                ...
                pd.DataFrame(user).to_csv("user.csv")
                pd.DataFrame(attendance).to_csv("attendance_from_device.csv")
                if attendance:
                    for each in attendance:
                        ...
                        for uid in user:
                            if uid.user_id == each.user_id:
                                ...
                                # if uid.name == 'M Ahmad latif':
                                _logger.info(f"uid.name: {uid.name}")
                                _logger.info(f"employee_id: {get_user_id.id}")
                                _logger.info(f"device_id_num: {each.user_id}")
                                _logger.info(f"attendance_type: {str(each.status)}")
                                _logger.info(f"punch_type: {str(each.punch)}")
                                _logger.info(f"punching_time: {atten_time}")
                                _logger.info(f"address_id: {info.address_id.id}")
                                _logger.info(f"each.punch: {each.punch}")
                                            ...
                                            if len(att_var) == 1:
                                                ...
                                                att_var1 = hr_attendance.search(
                                                    [('employee_id', '=',
                                                      get_user_id.id)])
                                                if att_var1:
                                                    _logger.info(f"Last Attendance Record: {att_var1[-1].read(['employee_id', 'check_in', 'check_out'])}")
                                                    _logger.info(f"=========== else if att_var1 === {att_var1[-1]} == {atten_time}")
                                                    # written = att_var1[-1].write({
                                                    #     'check_out': atten_time
                                                    # })
                                                    # _logger.info(f"=========== else if att_var1 === {written}")
                    ...
                    _logger.info(f"simple string ========= exit code")
                    conn.disconnect
                    return True
                else:
